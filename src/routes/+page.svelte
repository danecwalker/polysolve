<script lang="ts">
  ...
  let calculatePolynomial = $derived(
    (x: number[]) => x.map((_x) => {
        return {
          x: _x,
          y: model().fit(_x),
    ...
    >
      ...
      >
        <g fill="none" stroke="black" stroke-width="0.01">
          ...
          {/each}
          <path
            d={lineGenerator(calculatePolynomial(
              (() => { 
  ...
  >
    ...
    >
      <div class="flex justify-between items-center">
        ...
        <div>
          <input class="border border-gray-300 rounded p-2 text-center" type="number" name="np" id="np" min="1" max="100" value={nPoints} oninput={(e) => {
    ...
    >
      ...
      <div class="border border-gray-300 rounded overflow-hidden flex flex-col p-2">
        <!-- {#if poly().length > 0}
